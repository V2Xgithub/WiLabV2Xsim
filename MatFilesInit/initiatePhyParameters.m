...
phyParams.Raw = sort(phyParams.Raw, 'ascend');
if phyParams.Raw(1)<=0
...
phyParams.Gr = db2pow(phyParams.Gr_dB);
...
% Noise power per MHz
phyParams.Pnoise_MHz_dBm = pow2db(constants.BOLTZMANN_CONSTANT*constants.REFERENCE_TEMPERATURE*1e6) + phyParams.F_dB + 30;
phyParams.Pnoise_MHz = db2pow(phyParams.Pnoise_MHz_dBm-30);
...
if simParams.typeOfScenario == constants.SCENARIO_URBAN  % Urban-ETSI has NLOS PER curve
...
if simParams.technology ~= constants.TECH_ONLY_CV2X % not only C-V2X (lte or 5g)
    ...
    % Slot time (s)
    phyParams.tSlot = constants.TIME_11P_SLOT_SEC;
    ...
    phyParams.tAifs = constants.TIME_11P_SIFS_SEC + phyParams.tSlot * phyParams.AifsN;
    ...
    phyParams.PrxSensNotSynch = db2pow(phyParams.CCAthr11p_notsync-30);
    ...
    phyParams.PrxSensWhenSynch = db2pow(phyParams.CCAthr11p_sync-30);
    ...
    if phyParams.PERcurves
        ...
        if (simParams.typeOfScenario == constants.SCENARIO_URBAN)
        ...
        phyParams.sinrThreshold11p_NLOS_dB = pow2db(sinrInterpNLOS(find(perInterpNLOS>=0.9,1)));
        phyParams.sinrThreshold11p_LOS_dB = pow2db(sinrInterp(find(perInterp>=0.9,1)));
        ...
        if ~phyParams.pWithLTEPHY
            ...
            if phyParams.sinrThreshold11p_LOS == -1000
                phyParams.sinrThreshold11p_LOS_dB = autoSINRmin11p(phyParams.MCS_11p, appParams.beaconSizeBytes, phyParams.BwMHz);
    ...
    phyParams.sinrThreshold11p_LOS = db2pow(phyParams.sinrThreshold11p_LOS_dB);
    phyParams.sinrVector11p_LOS = db2pow(phyParams.sinrVector11p_LOS_dB);

    phyParams.sinrThreshold11p_NLOS = db2pow(phyParams.sinrThreshold11p_NLOS_dB);
    phyParams.sinrVector11p_NLOS = db2pow(phyParams.sinrVector11p_NLOS_dB);
...
if simParams.technology ~= constants.TECH_ONLY_11P % not only 11p
    ...
    elseif simParams.mode5G == constants.MODE_5G
        ...
        if ~ismember(phyParams.nDMRS_NR, [12, 15, 18, 21, 24])
        ...
        if ~ismember(phyParams.sizeSubchannel, [10,12,15,20,25,50,75,100])
    ...
    switch phyParams.duplexCV2X
        ...
        case 'FD'
            ...
            phyParams.Ksi = db2pow(phyParams.Ksi_dB);

            % [PDelta]
            if strcmp(phyParams.duplexCV2X,'FD')
                [phyParams,varargin]= addNewParam(phyParams,'PDelta',Inf,'Multiplicative SelfI factor to set FD reselection threshold','double',fileCfg,varargin{1});
            end
        otherwise
...
% [retransType]
[phyParams,varargin]= addNewParam(phyParams ,'retransType',0,'Retransmission type, 0: static, 1: deterministic, 2: probabilistic','integer',fileCfg,varargin {1}) ;
...
[phyParams,varargin] = addNewParam(phyParams,'channelModel',0,'Channel model (0:WINNER+ B1; N: N slopes with N=1,2,3; 4: 5G NLOSv model)','integer',fileCfg,varargin{1});
if ~ismember(phyParams.channelModel, [constants.CH_WINNER_PLUS_B1, constants.CH_SINGLE_SLOPE,...
        constants.CH_TWO_SLOPES, constants.CH_THREE_SLOPES, constants.CH_5G_NLOSV])
    error('Error: "phyParams.channelModel" must be between [0, 4]');
end

switch phyParams.channelModel
    case constants.CH_WINNER_PLUS_B1                            % WINNER+ B1
        h = 1.5;                                                % Antenna height [m]
        h_eff = h - 1;                                          % Effective antenna height [m]
        f = 5.9;                                                % Central frequency [GHz]
        Dbp = 4*h_eff*h_eff*f*10^9/constants.SPEED_OF_LIGHT;    % Breakpoint distance [m]
        
        phyParams.d_threshold1 = 0;
        phyParams.d_threshold2 = Dbp;
        phyParams.L0_near = 1000;
        phyParams.b_near = 0;
        phyParams.L0_mid = db2pow(27.0 + 20.0*log10(f));
        phyParams.b_mid = 2.27;
        phyParams.L0_far = db2pow(7.56-34.6*log10(h_eff)+2.7*log10(f));
        phyParams.b_far = 4;
        ...
        % NLOS
        if simParams.typeOfScenario == constants.SCENARIO_URBAN %ETSI Manhattan Layout
    ...
    case constants.CH_SINGLE_SLOPE % Single Slope
        % [L0]
        % Path loss at 1m (dB)
    ...
    case constants.CH_THREE_SLOPES
        % [dthreshold1]
        % threshold distance
        [phyParams,varargin] = addNewParam(phyParams,'d_threshold1',10,'First distance threshold','double',fileCfg,varargin{1});
        if phyParams.d_threshold1 < 1
...
if phyParams.channelModel ~= constants.CH_WINNER_PLUS_B1  % NLOS non-Winner
    % [Abuild]
    ...
    phyParams.Abuild = db2pow(phyParams.Abuild_dB);
    ...
    phyParams.Awall = db2pow(phyParams.Awall_dB);
