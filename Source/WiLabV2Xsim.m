...
outputValues.computationTime = -1;
% column vector of blocking rates at each awareness range
outputValues.blockingRateCV2X = -1*ones(1,length(phyParams.Raw));
outputValues.blockingRate11p = -1*ones(1,length(phyParams.Raw));
outputValues.blockingRateTOT = -1*ones(1,length(phyParams.Raw));
% column vector of error rates at each awareness range
outputValues.errorRateCV2X = -1*ones(1,length(phyParams.Raw));
outputValues.errorRate11p = -1*ones(1,length(phyParams.Raw));
outputValues.errorRateTOT = -1*ones(1,length(phyParams.Raw));
% column vector of packet reception ratios at each awareness range
outputValues.packetReceptionRatioCV2X = -1*ones(1,length(phyParams.Raw));
outputValues.packetReceptionRatio11p =  -1*ones(1,length(phyParams.Raw));
outputValues.packetReceptionRatioTOT = -1*ones(1,length(phyParams.Raw));
% number of UNIQUE vehicles seen the entire simulation
outputValues.NvehiclesLTE = 0;
outputValues.Nvehicles11p = 0;
outputValues.NvehiclesTOT = 0;
% column vector of average number of neighbors at each awareness range
outputValues.NneighborsCV2X = zeros(1,length(phyParams.Raw));
outputValues.Nneighbors11p = zeros(1,length(phyParams.Raw));
outputValues.NneighborsTOT = zeros(1,length(phyParams.Raw));
% column vector of standard deviation of the number of neighbors at each
% awareness range
outputValues.StDevNeighboursCV2X = zeros(1,length(phyParams.Raw));
outputValues.StDevNeighbours11p = zeros(1,length(phyParams.Raw));
outputValues.StDevNeighboursTOT = zeros(1,length(phyParams.Raw));
% number of reassignment events
outputValues.NreassignCV2X = 0;
% 3D matrix of number of blocked packets per channel per packet type per
% awareness range
outputValues.NblockedCV2X = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
outputValues.Nblocked11p = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
outputValues.NblockedTOT = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
% 3D matrix of number of transmitted beacons per channel per packet type
% per awareness range
outputValues.NtxBeaconsCV2X = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
outputValues.NtxBeacons11p = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
outputValues.NtxBeaconsTOT = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
% 3D matrix of number of error'd beacons per channel per packet type per
% awareness range
outputValues.NerrorsCV2X = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
outputValues.Nerrors11p = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
outputValues.NerrorsTOT = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
% 3D matrix of number of correctly transmitted beacons per channel per
% packet type per awareness range
outputValues.NcorrectlyTxBeaconsCV2X = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
outputValues.NcorrectlyTxBeacons11p = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
outputValues.NcorrectlyTxBeaconsTOT = zeros(phyParams.nChannels,appParams.nPckTypes,length(phyParams.Raw));
% scalar number of CV2X transmissions for HARQ
outputValues.cv2xTransmissionsIncHarq = 0;
outputValues.cv2xTransmissionsFirst = 0;
