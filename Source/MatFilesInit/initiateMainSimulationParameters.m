...
switch simParams.typeOfScenario
    ...
    case constants.SCENARIO_HIGHWAY  % ETSI Highway high speed
        ...
        [simParams,varargin] = addNewParam(simParams,'laneWidth',4,'Road Width (m)','double',fileCfg,varargin{1});
        if simParams.laneWidth<0
            error('Error: "simParams.laneWidth" cannot be < 0');
        end
        
        % [laneGap]
        % Gap between each lane (m)
        [simParams,varargin] = addNewParam(simParams,'laneGap',0,'Lane Gap (m)','double',fileCfg,varargin{1});
        if simParams.laneGap<0
            error('Error: "simParams.laneGap" cannot be < 0');
        end
        ...
        [simParams,varargin] = addNewParam(simParams,'vStdDev',0,'Standard deviation of speed of vehicles (Km/h)','double',fileCfg,varargin{1});
        if simParams.vStDev<0
            error('Error: "simParams.vStdDev" cannot be < 0');
        end
