...
if isfield(stationManagement,'IDvehicleTXLTE') && ~isempty(stationManagement.transmittingIDsCV2X)
    isLTEtxLeft = stationManagement.transmittingIDsCV2X(logical(logical_direction(stationManagement.transmittingIDsCV2X)));
    plot(positionManagement.XvehicleReal(isLTEtxLeft),...
        positionManagement.YvehicleReal(isLTEtxLeft),'<m','MarkerSize',8);
end
isLTERigth = logical(~logical_direction.*(stationManagement.vehicleState==100));
plot(positionManagement.XvehicleReal(isLTERigth),...
    positionManagement.YvehicleReal(isLTERigth),'>r');
if isfield(stationManagement,'IDvehicleTXLTE') && ~isempty(stationManagement.transmittingIDsCV2X)
    isLTEtxRigth = stationManagement.transmittingIDsCV2X(logical(~logical_direction(stationManagement.transmittingIDsCV2X)));
